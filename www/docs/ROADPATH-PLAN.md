...
- [x] `radius[]` и `safeSpeed[]` в RouteGeometry (`js/curve-speed.js`).
- [x] Физика: `v = sqrt(a_lat · R)`; пресеты **мягче / норма / строже** (0.28–0.35 g); уклон вниз ужесточает оценку.

**Индикация:**

- [x] **Окраска ленты** жёлтый/красный по `v/v_safe` только в **зоне подхода** (до входа); нейтральный = нет предупреждения.
- [x] **Голос** за ~3–7 с хода до шпильки при превышении.
- [x] **Внутри поворота** индикация отключена.

**Защита от ложных срабатываний:**

- [x] Минимальная длина дуги 25 м; только **`R < 100 м`**.
- [x] Настройка «строже / мягче»; **выключение** чекбоксом.
- [ ] Гистерезис порогов (бэклог v1.1).
...
- [x] Карта рисует маршрут из dense `RouteGeometry` (fallback — OSRM coords до сборки).
- [x] Маркеры поворотов и дистанции привязаны к `s` / geometry snap.
- [x] Превью «окна HUD» на setup (длина из настроек, срез по `s`).
- [x] Polyline карты = `dense` после `scheduleGeometryBuild`.
- [x] АЗС на дорожке: проекция и `distAhead` в пространстве `s`.
