...
| **pre-Valhalla** | `yandexnavi://` экспорт, GPX-трек 1 Гц, `baseline_v1.json` | ✅ v3.1 |
...
## Финансирование (отдельно от навигации)

ПО **бесплатное**; paywall запрещён. Добровольное вознаграждение — по [публичной оферте](offer.html) через СБП на странице [поддержки](support.html). В сборках для RuStore/NashStore/RuMarket ссылка на оплату **не показывается в UI** (только в описании карточки). Подробности: [`HANDOFF-SBP-LEGAL.md`](HANDOFF-SBP-LEGAL.md).

---

**Итог v3:** направления 15–16 закрывают российский UX «построил в Яндексе — веду в Мото ИЛС» и тактический обзор карты без выхода из HUD. Реализация **не блокируется** Valhalla и может идти параллельно S1–S2.

**Итог v3.1 (pre-Valhalla):** добавлены экспорт маршрута в Яндекс Навигатор (`js/yandex-export.js`), локальная запись трека с GPX (`js/track-recorder.js`), целевой baseline S4 (`fixtures/baseline_v1.json`). **Следующий шаг:** прогон 5 треков телеметрии → `npm run nav:baseline`, затем **№7 Valhalla** (Docker + `trace_route`).
